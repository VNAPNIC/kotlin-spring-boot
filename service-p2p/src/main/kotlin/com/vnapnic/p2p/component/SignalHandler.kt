    ...
    override fun handleTextMessage(session: WebSocketSession, message: TextMessage) {
        ...
        try {
            ...
            when (wsMessage.type) {
                ...
                MSG_TYPE_TEXT -> {
                    log.info("[ws] Text message: {}", wsMessage.data)
                    // message.data is the text sent by client
                    // process text message if needed
                    session.sendMessage(TextMessage("reollback: ${wsMessage.data}"))
                }
                ...
                MSG_TYPE_OFFER, MSG_TYPE_ANSWER, MSG_TYPE_ICE -> {
                    ...
                    log.info("[ws] Signal: {}", candidate?.toString()?.substring(0, 64) ?: sdp?.toString()?.substring(0, 64))

                    sessionIdToRoomMap[session.id]?.let { room ->
                ...
                MSG_TYPE_JOIN -> {
                    ...
                    log.info("[ws] {} has joined Room: #{}", userName, data)
                    ...
                    session.sendMessage(TextMessage("session: ${session.id}"))
                }
                MSG_TYPE_LEAVE -> {
                    // message data contains connected room id
                    log.info("[ws] {} is going to leave Room: #{}", userName, wsMessage.data)
